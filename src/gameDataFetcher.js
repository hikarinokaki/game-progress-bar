...
async function getAuthToken() {
  ...
  if (
    authToken &&
    authTokenTimestamp &&
    Date.now() - authTokenTimestamp < AUTH_TOKEN_DURATION
  ) {
  ...
  try {
    ...
    const tokenCookie = cookies.find(
      (c) => c.name === "x-auth-token" || c.name.includes("auth"),
    );
    if (tokenCookie) {
...
async function getGameMaxTime(appid, gameName) {
  ...
  try {
    ...
    if (!token) {
      throw new Error(
        "Failed to obtain authentication token from HowLongToBeat",
      );
    }
    ...
    if (error.response) {
      ...
      console.error(
        `Error setting up request for "${gameName}":`,
        error.message,
      );
    }
